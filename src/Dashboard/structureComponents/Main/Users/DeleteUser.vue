<template>
	<Dialog class="delete-dialog" v-model:visible="showMessage" :modal="true" :breakpoints="{ '960px': '80vw' }" :style="{ width: '30vw' }" position="center">
		<div class="d-flex align-items-center flex-row pt-6 px-3">
			...
			<h5 v-if="!deleteMessage" class="mt-3 ms-4">Do you really want to delete user: <b>{{ userName }}</b></h5>
			<h5 v-if="deleteMessage" class="mt-3 ms-4">{{ deleteMessage }}: {{ userName }}</h5>
		</div>
		<template #footer>
...
<script>
	export default {
		...
		methods: {
			...
			deleteUser() {				
				axios.delete( DOMAIN_URL + '/api/users/' + this.userId, {
						headers: {
