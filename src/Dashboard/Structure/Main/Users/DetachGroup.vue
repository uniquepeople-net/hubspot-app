	...
	<div>
		...
		<div class="row">
			<div class="col-12 col-md-6 col-xl-4">
				<Card class="card">
					<template #title>
						<h5 class="card-header">{{ $t('message.Groups') }}</h5>
					</template>
					<template #content>
						<MultiSelect v-model="selectedGroups" :options="groupsToDeatch(users)" class="multi-select" 
									 optionLabel="name" :placeholder="$t('message.SelectGroup')"/>
					</template>
					<template #footer>
			...
			<div class="col-12 col-md-6 col-xl-4">
				<Card class="card h-100">
					<template #title>
						<h5 class="card-header">{{ $t('message.Choosed') + ' ' + $t('message.users') }}</h5>
					</template>
					<template #content>
