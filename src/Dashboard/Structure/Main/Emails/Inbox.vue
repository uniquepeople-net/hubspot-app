<template>
	<div>
		...
		<DataTable v-if="emails" :value="emails" :paginator="true" class="p-datatable-customers card" :rows="10"
			...
			:globalFilterFields="['reply_name', 'reply_email', 'subject', 'created_at']" responsiveLayout="scroll" @row-click="handleRowClick($event)">
			<template #header>
			...
			<Column field="reply_email" :header="$t('message.Address')" sortable style="min-width: 8rem">
				<template #body="{data}">
					<p class="m-0 text-sm-message" :class="!checkIsReadEmail(data) && 'fw-bold'">{{data.reply_email}}</p>
				</template>
				<template #filter="{filterModel}">
			...
			<Column field="subject" :header="$t('message.Subject')" sortable style="min-width: 8rem">
				<template #body="{data}">
					<p class="m-0 text-sm-message" :class="!checkIsReadEmail(data) && 'fw-bold'">{{data.subject}}</p>
				</template>
				<template #filter="{filterModel}">
					<InputText type="text" v-model="filterModel.value" class="p-column-filter" placeholder="Search by email"/>
				</template>
...
<script>
	...
	export default {
		...
		components: { TriStateCheckbox, Calendar, BulkActions, BackButton }
	}
</script>
